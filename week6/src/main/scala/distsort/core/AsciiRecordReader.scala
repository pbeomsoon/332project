...
class AsciiRecordReader(file: File) extends RecordReader {
  ...
  override def readRecord(): Option[Record] = {
    ...
    if (line == null) {
      // EOF reached
      None
    } else if (line.length != 201) {
      // Invalid line length
      // Expected: 20 (key hex) + 1 (space) + 180 (value hex) = 201
      None
    } else if (line.charAt(20) != ' ') {
      // Space missing at position 20
      None
    } else {
      // Parse hex strings
      ...
      try {
        ...
        if (key.length != 10 || value.length != 90) {
          // Hex parsing resulted in wrong byte count
          None
        } else {
