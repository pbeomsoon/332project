...
class GrpcWorkerClient(
  ...
  // ⭐ Track whether channel was ever created (for proper cleanup)
   ...
   * ⭐ Only closes if channel was actually created (prevents lazy val initialization during cleanup)
   */
  override def close(): Unit = {
    // ⭐ Only close if channel was actually used (prevents creating channel just to close it)
    if (!channelCreated) {
      logger.debug(s"Channel to $targetHost:$targetPort was never created, skipping close")
      return
    }
    ...
    Try {
      if (!channel.isShutdown) {
        ...
        if (!channel.awaitTermination(1, TimeUnit.SECONDS)) {
          logger.debug(s"Channel to $targetHost:$targetPort didn't terminate, forcing shutdown")
          channel.shutdownNow()
          // Don't wait again - just force it
        }
      ...
      case Failure(ex) =>
        logger.debug(s"Error closing channel to $targetHost:$targetPort: ${ex.getMessage}")
    }
  }
}
