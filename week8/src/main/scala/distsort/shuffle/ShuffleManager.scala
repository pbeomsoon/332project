...
class ShuffleManager(
  ...
  /**
   * ⭐ FIX: Streaming partition with bulk write optimization
   * - Bulk read from chunks (10000 records at a time)
   * - Direct byte copy (no toBytes allocation)
   * - 8MB write buffers per partition
   */
  private def partitionSortedChunksToFiles(
    ...
    val RECORD_SIZE = 100
    val BULK_READ_SIZE = 10000  // 10000 records = 1MB per read

    System.err.println(s"[ShuffleManager] Partitioning ${chunks.size} chunks...")
    val startTime = System.currentTimeMillis()

    // Create temp files and output streams for each partition (lazy init)
    val partitionStreams = mutable.Map[Int, (File, BufferedOutputStream)]()
    var totalRecords = 0L

    def getOutputStream(partitionId: Int): BufferedOutputStream = {
      partitionStreams.getOrElseUpdate(partitionId, {
        val file = new File(fileLayout.getTempBasePath.toFile, s"shuffle-partition-$partitionId.tmp")
        file.getParentFile.mkdirs()
        val out = new BufferedOutputStream(new FileOutputStream(file), 256 * 1024)  // 256KB buffer (288 partitions × 256KB = 72MB)
        (file, out)
      })._2
    }
    ...
    try {
      // Bulk read buffer
      val readBuffer = new Array[Byte](BULK_READ_SIZE * RECORD_SIZE)

      chunks.foreach { chunk =>
        val fis = new FileInputStream(chunk)
        val bis = new BufferedInputStream(fis, 4 * 1024 * 1024)  // 4MB read buffer

        try {
          var done = false
          while (!done) {
            val bytesRead = bis.read(readBuffer)
            if (bytesRead <= 0) {
              done = true
            } else {
              ...
              while (i < recordsInBatch) {
                ...
                // Extract key (first 10 bytes) for partitioning
                val key = new Array[Byte](10)
                System.arraycopy(readBuffer, offset, key, 0, 10)

                val partitionId = partitioner.getPartition(key)
                // Write directly from buffer (no copy needed)
                getOutputStream(partitionId).write(readBuffer, offset, RECORD_SIZE)
      ...
      // Close all streams
      partitionStreams.values.foreach { case (_, out) =>
        out.close()
      }

      val elapsed = System.currentTimeMillis() - startTime
      System.err.println(s"[ShuffleManager] ✅ Partitioned $totalRecords records in ${elapsed}ms")

      // Return partition ID -> file mapping
      partitionStreams.map { case (pid, (file, _)) => pid -> file }.toMap

    } catch {
      case ex: Exception =>
        System.err.println(s"[ShuffleManager] ❌ Partitioning failed: ${ex.getMessage}")
        // Clean up on error
        partitionStreams.values.foreach { case (_, out) =>
  ...
  /**
   * ⭐ Bulk read partition file into Record sequence
   * Much faster than RecordReader (single read instead of per-record read)
   */
  private def readPartitionFileBulk(file: File): Seq[Record] = {
    import java.io.{BufferedInputStream, FileInputStream}
    val RECORD_SIZE = 100

    val fileSize = file.length()
    if (fileSize == 0) return Seq.empty

    val recordCount = (fileSize / RECORD_SIZE).toInt
    val records = new Array[Record](recordCount)

    val fis = new FileInputStream(file)
    val bis = new BufferedInputStream(fis, 4 * 1024 * 1024)  // 4MB buffer
    ...
    try {
      ...
      while (i < recordCount) {
        ...
        if (bytesRead == RECORD_SIZE) {
          ...
          System.arraycopy(buffer, 0, key, 0, 10)
          System.arraycopy(buffer, 10, value, 0, 90)
  ...
  private def sendPartitionFilesToWorkers(
    ...
    val startTime = System.currentTimeMillis()
    System.err.println(s"[ShuffleManager] Sending ${partitionFiles.size} partition files to workers...")
      ...
      if (targetWorker == workerIndex) {
        ...
        partitionFile match {
          case Some(file) if file.exists() =>
            // ⭐ Bulk read: 전체 파일을 한번에 읽어서 Record 배열 생성
            val records = readPartitionFileBulk(file)
    ...
    Future.sequence(sendFutures).map { _ =>
      val elapsed = System.currentTimeMillis() - startTime
      System.err.println(s"[ShuffleManager] ✅ Sent all partitions in ${elapsed}ms")
    }
  }
