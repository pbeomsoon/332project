...
class Master(
  ...
  def start(): Unit = {
    ...
    server = ServerBuilder
      .forPort(port)
      .maxInboundMessageSize(100 * 1024 * 1024) // 100MB max message (for large samples)
  ...
  def run(): Unit = {
    ...
    try {
      ...
      val workflowEndTime = System.currentTimeMillis()
      val totalTimeMs = workflowEndTime - workflowStartTime
      val totalTimeSec = totalTimeMs / 1000.0
      val minutes = (totalTimeSec / 60).toInt
      ...
      System.err.println("============================================================")
      System.err.println(s"[Master] Total execution time: ${minutes}m ${f"$seconds%.2f"}s ($totalTimeMs ms)")
      System.err.println("============================================================")
