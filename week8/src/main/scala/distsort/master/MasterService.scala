...
object PhaseTimeouts {
  val PHASE_SAMPLING: Int = 600      // 10 minutes (large datasets need more time)
  val PHASE_SORTING: Int = 1200      // 20 minutes
  val PHASE_SHUFFLING: Int = 1800    // 30 minutes (네트워크 느린 환경 대비)
  val PHASE_MERGING: Int = 1200      // 20 minutes
...
class MasterService(
  ...
  private val MAX_PARTITIONS_PER_WORKER: Int = 500  // Maximum partitions per worker (supports up to ~48GB with 3 workers)
  ...
  // Heartbeat tracking
  // ⭐ 빠른 crash 감지: 3초 × 4회 = 12초
  // - 너무 짧으면 false positive 발생
  // - 너무 길면 recovery 대기 시간 증가
  ...
  private val heartbeatCheckInterval = 5.seconds  // 체크 주기 (5초) - GC pause 대비
  private val MISSED_HEARTBEAT_THRESHOLD = 6  // 6회 연속 미수신 시 FAILED (30초) - 대용량 데이터 처리 대비
