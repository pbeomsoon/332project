...
class Worker(
   ...
   * ⭐ retries=10 for fault tolerance (총 ~63초, 크래시 감지 30초보다 충분히 김)
   */
  private def registerWithMaster(retries: Int = 10, delayMs: Long = 1000): Unit = {
    try {
   ...
   *
   * ⭐ Initial delay 2초로 단축 (기존 5초)
   * - Master가 12초 내 heartbeat 없으면 FAILED 처리하므로 빨리 시작해야 함
   */
  private def startHeartbeat(): Unit = {
    heartbeatRunning = true
    val initialDelaySeconds = 2L  // ⭐ 5초→2초 (빠른 heartbeat 시작)
  ...
  private def sendHeartbeat(): Unit = {
    ...
    try {
      ...
      val response = Await.result(responseFuture, 3.seconds)  // ⭐ 10초→3초 (blocking 방지)
    ...
    workerService.setShuffleDataRequestHandler { request =>
      ...
      try {
        ...
        recordRanges.foreach { range =>
          ...
          val RECORD_SIZE = 100
          val fis = new FileInputStream(range.file)
          ...
          try {
            bis.skip(range.startRecord * RECORD_SIZE)
            val buffer = new Array[Byte](RECORD_SIZE)
            var recordsRead = 0L
            while (recordsRead < range.recordCount) {
              ...
              if (bytesRead == RECORD_SIZE) {
                ...
                recordsRead = range.recordCount  // EOF
              }
            }
          } finally {
            bis.close()
  ...
  private def performMerge(): Unit = {
    ...
    System.err.println(s"[Merge] Received files: ${receivedFiles.size}")
    receivedFiles.foreach(f => System.err.println(s"[Merge]   - ${f.getAbsolutePath} (${f.length()} bytes)"))

    // Also include local partition files for assigned partitions
    System.err.println(s"[Merge] Assigned partitions: ${assignedPartitionsList.mkString(", ")}")
    val localPartitionFiles = assignedPartitionsList.map { partitionId =>
      fileLayout.getLocalPartitionFile(partitionId)
    }.filter(_.exists())
    System.err.println(s"[Merge] Local partition files: ${localPartitionFiles.size}")
    localPartitionFiles.foreach(f => System.err.println(s"[Merge]   - ${f.getAbsolutePath} (${f.length()} bytes)"))
    ...
    System.err.println(s"[Merge] Total files to merge: ${allFiles.size}")

    if (allFiles.nonEmpty) {
      // Merge using KWayMerger
      assignedPartitionsList.foreach { partitionId =>
        // Filter files for this partition
        // ⭐ FIX: Use exact match to avoid partition.3 matching partition.30!
        // Local: partition.3 (exact match)
        // Received patterns:
        //   - partition.3.received (old ShuffleBuffer format)
        //   - partition-3-abc12345.received (new streaming format with UUID)
        val partitionFiles = allFiles.filter { file =>
          val name = file.getName
          name == s"partition.$partitionId" ||                              // local partition file
          name == s"partition.$partitionId.received" ||                     // old received format
          name.matches(s"partition-$partitionId-[a-f0-9]{8}\\.received")    // new streaming format (8-char hex UUID)
        }

        System.err.println(s"[Merge] Partition $partitionId: ${partitionFiles.size} files")

        if (partitionFiles.nonEmpty) {
          val outputFile = fileLayout.getOutputPartitionFile(partitionId)

          // ⭐ FIX: Use KWayMerger for proper sorted merge (not simple append!)
          val merger = new KWayMerger(partitionFiles)
          merger.mergeToFile(outputFile)

          val recordCount = outputFile.length() / 100  // 100 bytes per record
          System.err.println(s"[Merge] Created ${outputFile.getName} with $recordCount records (K-way merged)")
          logger.info(s"Created output partition: ${outputFile.getAbsolutePath}")
        } else {
          System.err.println(s"[Merge] WARNING: No files for partition $partitionId!")
        }
      }
    } else {
      System.err.println(s"[Merge] WARNING: No files to merge!")
    }
