...
class GracefulShutdownManager(
   ...
   * Uses synchronous shutdown to avoid RejectedExecutionException
   */
    ...
    Runtime.getRuntime.addShutdownHook(new Thread() {
      override def run(): Unit = {
        if (!isShuttingDown) {
          ...
          logger.info("JVM shutdown hook triggered - using synchronous shutdown")

          // Use synchronous shutdown to avoid executor issues
          try {
            val components = componentsToShutdown.synchronized {
              componentsToShutdown.reverse.toList
            }

            components.foreach { component =>
              try {
                logger.info(s"Force shutting down: ${component.getClass.getSimpleName}")
                component.forceShutdown()
              } catch {
                case ex: Exception =>
                  logger.warn(s"Error during force shutdown: ${ex.getMessage}")
              }
            }
            ...
            case ex: Exception =>
              logger.error(s"Error during shutdown hook: ${ex.getMessage}")
          }
        }
      }
    })
