...
class KWayMerger(val sortedChunks: Seq[File]) {
   ...
   * ⭐ Bulk write 최적화: 10000 레코드씩 버퍼에 모아서 한번에 write
   *
  ...
  def mergeToFile(outputFile: File): Unit = {
    ...
    val out = new BufferedOutputStream(new FileOutputStream(outputFile), 8 * 1024 * 1024)  // 8MB buffer
    try {
      mergeWithCallback { record =>
        // Copy to buffer using System.arraycopy (no allocation)
        System.arraycopy(record.key, 0, buffer, bufferIndex * RECORD_SIZE, 10)
        System.arraycopy(record.value, 0, buffer, bufferIndex * RECORD_SIZE + 10, 90)
        bufferIndex += 1
